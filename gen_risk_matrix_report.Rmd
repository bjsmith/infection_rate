...
```{r}
#library(ggplot2)
ggplot(covid_data_to_save %>% filter(Location %in% key_interest_countries) %>% filter(Population>10^6 & `Life Expectancy`>=70),aes(x=`Estimated Prevalence (per million)`,y=slope_7days,label=Location))+
