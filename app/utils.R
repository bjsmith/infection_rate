...
get_data_closure <- function() {
  ...
  f <- function(force_reset=FALSE) {
    if(is.null(data_list) || (force_reset==TRUE)){
      ...
      get_cache_or_live_data<-function(live_data_function,cache_filepath,cache_expiry_in_minutes=60){
        if(file.exists(cache_filepath)){
          if(as.double(difftime(Sys.time(),file.info(cache_filepath)$mtime,units="mins"))<cache_expiry_in_minutes){
            ...
            #use it
            returned_data <- readRDS(file = cache_filepath)
      ...
      dl_local[["jh_data"]] <- get_cache_or_live_data(retrieve_jh_data,"data/jh_cache.Rds",cache_expiry_in_minutes = 120)
      ...
      google_sheets_cache_filepath <- "data/manual_corrections_cache.Rds"
      ...
      owid_cache_filepath <- "data/owid_cache.Rds"
