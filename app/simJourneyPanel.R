  ...
  return(
    wellPanel(
      ...
      numericInput(paste0("intsim_percent_tvolume_level",level_id),
                   "Traveller volume (% of 2019 levels over current levels):",
                   min=1,
                   max=100,step=1,
                   value = default_volume),
      conditionalPanel(condition = "input.simsettings_mode=='Advanced'",
        numericInput(paste0("intsim_effectiveness_level",level_id),
                     "Effectiveness (% of cases entering the traveller journey that do NOT exit MIQ into the community, before accounting for extra spread):",
                     min=0,
                     max=100,step=1,
                     value = default_effectiveness*100),
        numericInput(paste0("intsim_extraspread_level",level_id),
                     "Extra spread (% of cases entering the traveller journey that lead to a separate infection exiting MIQ):",
                     min=0,
                     max=100,step=0.1,
