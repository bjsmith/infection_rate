...
This month last year, New Zealand received `r scale_signif(params$location_info$LocationResidentMonthlyArrivalsScaled1) ` arrivals who were _resident_ in `r locname`. This includes citizens of that country residing there, and citizens of New Zealand and other countries who reside in `r locname`. Over the same time there were also `r scale_signif(params$location_info$NZResMonthlyArrivalsScaled1) ` New Zealand residents returning to New Zealand whose main destination while away was `r locname`. By adding these figures together we can roughly estimate the number of people who arrive in New Zealand per month after being in `r locname` in the previous 14 days at `r scale_signif(params$location_info$MonthlyArrivalsWeighted) `.
