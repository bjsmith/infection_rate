...
Two weeks ago, `r locname` had `r scale_signif(params$location_info$LaggedNewCases*7,2)` new cases over a seven day period. Over the last week, there have been `r signif(params$location_info$NewDeaths*7)` new deaths. This is a case fatality rate of `r signif(params$location_info$NewDeaths/params$location_info$LaggedNewCases*100,2)`%. We assume the actual infection to fatality rate, including undetected infections, is actually `r params$assumed_ifr*100`%, so we estimate that `r locname` is officially detecting `r signif(params$location_info$InferredDetectionRate*100,2)`% of their cases. Considering this detection rate, and the current number of official active confirmed cases, we estimate the true number of infections currently in `r locname` is closer to `r scale_signif(params$location_info$InferredActiveCases)`.
...
if(params$trust_rating=="trustworthy"){
  risk_statement <- paste0(
      ...
      locname,
      " would be ",
      as.character(signif(params$location_info$ExpectedNumberOfCasesAll,1)),
      " every month.",
      "\n<br /><br />  Considering New Zealand's quarantine system as it has existed over the last few months, a quarantine system could ensure that only around ",
      as.character(signif(params$location_info$ExpectedNumberOfCasesInCommunity,1)),
